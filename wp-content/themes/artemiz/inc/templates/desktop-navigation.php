...
if ( $navigationWidth == 'full' ) {
	if ( $paddingLeftRightUnit != 'px' && $paddingLeftRightUnit != '%' ) {
		$paddingLeftRightUnit = 'px';
	}
...
if ( $navigationSkin != 'dark' && $navigationSkin != 'light' && $navigationPos != '' ) {
	$navigationPos = 'absolute';
}
	...
	foreach ( $artemiz_fn_option['one_sorter']['left'] as $key => $value ) {
		...
		switch ( $key ) {
			case '':
				break;
			case 'menu':
				$left .= '<div class="tt_left_item tt_item_' . $key . '">' . $menu . '</div>';
				break;
			case 'search':
				$left .= '<div class="tt_left_item tt_item_' . $key . '">' . $search . '</div>';
				break;
			case 'social':
				$left .= '<div class="tt_left_item tt_item_' . $key . '">' . $socialList . '</div>';
				break;
			case 'logo':
				$left .= '<div class="tt_left_item tt_item_' . $key . '">' . $logo . '</div>';
				break;
		}
	}
}
	...
	foreach ( $artemiz_fn_option['one_sorter']['center'] as $key => $value ) {
		...
		switch ( $key ) {
			case '':
				break;
			case 'menu':
				$center .= '<div class="tt_center_item tt_item_' . $key . '">' . $menu . '</div>';
				break;
			case 'search':
				$center .= '<div class="tt_center_item tt_item_' . $key . '">' . $search . '</div>';
				break;
			case 'social':
				$center .= '<div class="tt_center_item tt_item_' . $key . '">' . $socialList . '</div>';
				break;
			case 'logo':
				$center .= '<div class="tt_center_item tt_item_' . $key . '">' . $logo . '</div>';
				break;
		}
	}
}
	...
	foreach ( $artemiz_fn_option['one_sorter']['right'] as $key => $value ) {
		...
		switch ( $key ) {
			case '':
				break;
			case 'menu':
				$right .= '<div class="tt_right_item tt_item_' . $key . '">' . $menu . '</div>';
				break;
			case 'search':
				$right .= '<div class="tt_right_item tt_item_' . $key . '">' . $search . '</div>';
				break;
			case 'social':
				$right .= '<div class="tt_right_item tt_item_' . $key . '">' . $socialList . '</div>';
				break;
			case 'logo':
				$right .= '<div class="tt_right_item tt_item_' . $key . '">' . $logo . '</div>';
				break;
		}
	}
}
        ...
        <div class="fn_container">
            <div class="search_box">
                <form action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get">
                    <input type="text" placeholder="<?php esc_attr_e( 'Search', 'artemiz' ); ?>" name="s"
                           autocomplete="off"/>
